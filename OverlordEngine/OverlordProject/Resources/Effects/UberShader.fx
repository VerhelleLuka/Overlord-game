...
{
		float3 halfVector;	
	halfVector = -normalize(viewDirection + gLightDirection); 
	float specAngle = max(dot(halfVector, normal), 0.0);
	
	return pow(specAngle, gShininess);
}
...
float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	reflectedVector = reflect(-gLightDirection, normal);
	float specAngle = max(dot(reflectedVector, viewDirection), 0.0);
...
float3 CalculateSpecular(float3 viewDirection, float3 normal, float2 texCoord)
{
		...
		if(gUseSpecularPhong)
	{
		if (!gUseTextureSpecularIntensity)
			return gColorSpecular * CalculateSpecularPhong(viewDirection, normal, texCoord);
		else
			return gTextureSpecularIntensity.Sample(gTextureSampler, texCoord) * CalculateSpecularPhong(viewDirection, normal, texCoord);

	}
	else
	{
		if (!gUseTextureSpecularIntensity)
			return gColorSpecular * CalculateSpecularBlinn(viewDirection, normal, texCoord);
		else
			return gTextureSpecularIntensity.Sample(gTextureSampler, texCoord) * CalculateSpecularBlinn(viewDirection, normal, texCoord);
...
float3 CalculateNormal(float3 tangent, float3 normal, float2 texCoord)
{
	...
	{	
		...
		newNormal = float3((2* newNormal.x)-1,
		 (2* newNormal.y)-1,
		 (2* newNormal.z)-1);
		//newNormal.y = (2* newNormal.y)-1;
		//newNormal.z = (2* newNormal.z)-1;
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
	...
	float result = fresnel * fresnelMask;
	environmentColor *= result;
	}
...
{
	float environmentColor = float3(0, 0, 0);
	
	if(gUseEnvironmentMapping)
	{
		float3 reflectedvector = normalize(reflect(viewDirection, normal) * gReflectionStrength + refract(viewDirection, normal, gRefractionIndex) * gRefractionStrength);
		
		environmentColor = gCubeEnvironment.Sample(gTextureSampler,reflectedvector);
	}
	return environmentColor;
}
