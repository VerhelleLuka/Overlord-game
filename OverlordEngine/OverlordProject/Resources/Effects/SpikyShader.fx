...
void SpikeGenerator(triangle VS_DATA vertices[3], inout TriangleStream<GS_DATA> triStream)
{
    ...
    float2 emptyTexCoord = float2(0, 0);
    //FACE 1
    float3 edge11 = vertices[1].Position - vertices[0].Position;
    float3 edge12 = basePoint - vertices[0].Position;
    float3 faceNormal1 = normalize(cross(edge11, edge12));

    CreateVertex(triStream, basePoint, faceNormal1, emptyTexCoord);
    CreateVertex(triStream, vertices[0].Position, faceNormal1, emptyTexCoord);
    CreateVertex(triStream, vertices[1].Position, faceNormal1, emptyTexCoord);

    //Restart Strip! >> We want to start a new triangle (= interrupt trianglestrip)
    triStream.RestartStrip();

    //FACE 2
    float3 edge21 = vertices[2].Position - vertices[1].Position;
    float3 edge22 = basePoint - vertices[1].Position;
    float3 faceNormal2 = normalize(cross(edge21, edge22));
    CreateVertex(triStream, basePoint, faceNormal2, emptyTexCoord);
    CreateVertex(triStream, vertices[1].Position, faceNormal2, emptyTexCoord);
    CreateVertex(triStream, vertices[2].Position, faceNormal2, emptyTexCoord);

    //Face 3
    triStream.RestartStrip();
    float3 edge31 = vertices[0].Position - vertices[2].Position;
    float3 edge32 = basePoint - vertices[2].Position;
    float3 faceNormal3 = normalize(cross(edge31, edge32));
    CreateVertex(triStream, basePoint, faceNormal3, emptyTexCoord);
    CreateVertex(triStream, vertices[2].Position, faceNormal3, emptyTexCoord);
    CreateVertex(triStream, vertices[0].Position, faceNormal3, emptyTexCoord);
